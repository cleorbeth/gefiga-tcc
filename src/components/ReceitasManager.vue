...
<template>
  <div class="max-w-4xl mx-auto bg-white p-6 rounded shadow border-l-4 border-green-500">
      ...
      <form @submit.prevent="saveReceita" class="bg-green-50 p-6 rounded-lg mb-6 flex flex-col md:flex-row gap-4 items-end shadow-sm border border-green-100">
          ...
          <button class="bg-green-600 text-white px-6 py-2 rounded font-bold h-10 hover:bg-green-700 shadow transition w-full md:w-auto">
              {{ form.id ? 'Salvar' : 'Adicionar' }}
          </button>
          <button v-if="form.id" @click="resetForm" type="button" class="bg-gray-300 text-gray-700 px-4 py-2 rounded font-bold h-10 hover:bg-gray-400 transition">
              X
          </button>
      </form>

      <ul class="space-y-3">
          <li v-for="r in receitas" :key="r.id" class="flex justify-between items-center p-4 bg-white border border-gray-200 rounded hover:shadow-sm transition border-l-4 border-l-green-500">
