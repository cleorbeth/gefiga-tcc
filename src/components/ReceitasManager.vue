...
onMounted(() => {
    try {
        ...
        onSnapshot(collection(db, 'artifacts', appId, 'users', uid, 'receitas'), (snap) => {
            receitas.value = snap.docs.map(d => ({ ...d.data(), id: d.id }));
        });
    ...
    <div class="max-w-4xl mx-auto bg-white p-6 rounded shadow border-l-4 border-green-500">
        <div class="flex justify-between items-center mb-6">
            ...
            <button @click="emit('back')"
                class="bg-gray-200 px-4 py-2 rounded text-gray-700 hover:bg-gray-300 font-bold text-sm">Voltar</button>
        </div>

        <form @submit.prevent="saveReceita"
            class="bg-green-50 p-6 rounded-lg mb-6 flex flex-col md:flex-row gap-4 items-end shadow-sm border border-green-100">
            <div class="flex-grow w-full">
                <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Descrição</label>
                <input v-model="form.descricao"
                    class="w-full p-2 border border-gray-300 rounded bg-white text-gray-900 outline-none focus:ring-2 focus:ring-green-500"
            ...
            <button
                class="bg-green-600 text-white px-6 py-2 rounded font-bold h-10 hover:bg-green-700 shadow transition w-full md:w-auto">
                {{ form.id ? 'Salvar' : 'Adicionar' }}
            </button>
            <button v-if="form.id" @click="resetForm" type="button"
                class="bg-gray-300 text-gray-700 px-4 py-2 rounded font-bold h-10 hover:bg-gray-400 transition">
                X
            </button>
        </form>
        ...
        <ul class="space-y-3">
            <li v-for="r in receitas" :key="r.id"
                class="flex justify-between items-center p-4 bg-white border border-gray-200 rounded hover:shadow-sm transition border-l-4 border-l-green-500">
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <div class="flex gap-2">
                        <button @click="editItem(r)"
                            class="bg-gray-100 text-blue-600 text-xs px-3 py-1 rounded border hover:bg-blue-50 font-bold">Editar</button>
                        <button @click="deleteItem(r.id)"
                            class="bg-gray-100 text-red-600 text-xs px-3 py-1 rounded border hover:bg-red-50 font-bold">X</button>
                    </div>
                </div>
            </li>
        </ul>
