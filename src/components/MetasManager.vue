    ...
    <div class="max-w-5xl mx-auto bg-white p-6 rounded shadow border-l-4 border-blue-500">
        ...
        <form @submit.prevent="saveMeta" class="bg-blue-50 p-6 rounded-lg mb-8 grid gap-4 md:grid-cols-2 shadow-sm border border-blue-100">
            ...
            <div class="md:col-span-2 flex gap-2 mt-2">
                <button type="submit" class="flex-grow bg-blue-600 text-white py-2 rounded hover:bg-blue-700 font-bold shadow">{{ form.id ? 'Salvar Alterações' : 'Criar Nova Meta' }}</button>
                <button v-if="form.id" @click="resetForm" type="button" class="bg-gray-300 px-6 rounded text-gray-700 font-bold hover:bg-gray-400">Cancelar</button>
            </div>
        </form>

        <div class="grid gap-6 md:grid-cols-2">
            <div v-for="m in props.listaMetas" :key="m.id" class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:shadow-md transition">
                ...
                <div class="bg-blue-50 p-3 rounded-lg border border-blue-100">
                    ...
                    <div class="flex items-center gap-3">
                        <input type="range" min="0" :max="props.saldo > 0 ? props.saldo : 1000" step="10" 
                            v-model.number="tempInvestment[m.id]" class="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-blue-600">
